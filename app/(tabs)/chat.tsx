...
import { MessageSquare, Send, Copy, RefreshCw, Bot, User, ChevronRight, CircleAlert as AlertCircle } from 'lucide-react-native';
...
export default function ChatScreen() {
  ...
  const handleUseReply = () => {
    if (replies[activeReply]) {
      setHistory(prev => [{ msg: receivedMsg, reply: replies[activeReply], style: replyStyle }, ...prev]);
      setReceivedMsg('');
      setContext('');
      setReplies([]);
    }
          ...
          <View style={[styles.styleGrid, isRTL && styles.styleGridRTL]}>
            {replyStyles.map(style => (
              <TouchableOpacity
                ...
                style={[
                  styles.styleCard,
                  replyStyle === style.key && { borderColor: style.color, backgroundColor: `${style.color}18` },
                ]}
                activeOpacity={0.75}
              >
                <Text style={[styles.styleLabel, { color: replyStyle === style.key ? style.color : colors.textPrimary }]}>
                  {style.label}
                </Text>
          ...
          <GradientCard variant="primary" style={styles.replyCard}>
            <View style={[styles.replyHeaderRow, rtlRow]}>
              <View style={[styles.aiTag, rtlRow]}>
                ...
                <Text style={styles.aiTagText}>{lang === 'ar' ? 'توأمك الذكي' : 'AI Twin'}</Text>
              </View>
              {styleInfo && (
                <View style={[styles.styleBadge, { backgroundColor: `${styleInfo.color}22`, borderColor: styleInfo.color }]}>
            ...
            <View style={[styles.replyActions, rtlRow]}>
              <TouchableOpacity onPress={handleCopy} style={styles.replyActionBtn}>
                ...
                <Text style={[styles.replyActionText, { color: copied ? colors.primary : colors.textMuted }]}>
              ...
              {replies.length > 1 && (
                ...
                >
                  ...
                  <Text style={[styles.replyActionText, { color: colors.accent }]}>{tr.tryAnotherBtn}</Text>
                </TouchableOpacity>
              )}
            ...
            {history.slice(0, 3).map((item, idx) => (
              <GradientCard key={idx} style={styles.historyCard}>
                <View style={[styles.historyMsgRow, rtlRow]}>
                  ...
                  <Text style={[styles.historyMsg, rtlText]} numberOfLines={2}>{item.msg}</Text>
                </View>
                <View style={[styles.historyReplyRow, rtlRow]}>
                  <Bot size={12} color={colors.primary} />
                  <Text style={[styles.historyReply, rtlText]} numberOfLines={2}>{item.reply}</Text>
                </View>
              </GradientCard>
            ))}
          </View>
